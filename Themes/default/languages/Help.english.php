...
$helptxt['http_dns_prefetch_domains'] = 'List of domains to prefetch DNS records for. You should only list external domains and not the domain this site is running on. The list is one domain per line and need to be in the format of //{DOMAIN} or {SCHEMA}://{DOMAIN}.';
$helptxt['http_dns_preconnect_domains'] = 'List of domains to preconnect to. You should only list the domains that you absolutely have a need to preconnect to, for example for external resources like webfonts that you use that are used in every page, otherwise use just dns-prefetch for external resources as overuse of preconnect is counterproductive. The list is one domain per line and needs to be in the format of {SCHEMA}://{DOMAIN}.<br><br>The crossorigin attribute can be set where needed for a domain by appending the word crossorigin after the domain name, seperated by a single space eg {SCHEMA}://{DOMAIN} crossorigin';
