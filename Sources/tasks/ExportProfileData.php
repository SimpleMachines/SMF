...
class ExportProfileData extends BackgroundTask
{
	 ...
	 */
	private array $xslt_stylesheet = array(
	...
	public function execute()
	{
		...
		$hook_methods = array(
			'pre_parsebbc' => in_array($this->_details['format'], array('HTML', 'XML_XSLT')) ? 'pre_parsebbc_html' : 'pre_parsebbc_xml',
	...
	protected function buildStylesheet(): void
	{
		...
		if (in_array($this->_details['format'], array('HTML', 'XML_XSLT')))
		{
	...
	public static function pre_parsebbc_xml(&$message, &$smileys, &$cache_id, &$parse_tags, &$cache_key_extras)
	{
		...
		else
		{
			...
			Config::$modSettings['disabledBBC'] = implode(',', array_unique(array_merge(array_filter(explode(',', Config::$modSettings['disabledBBC'])), array('attach'))));
		}
	}
