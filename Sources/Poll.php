...
class Poll implements \ArrayAccess
{
	...
	const LOAD_BY_ID = 0;
	const LOAD_BY_TOPIC = 1;
	const LOAD_BY_RECENT = 2;
	const LOAD_BY_VOTES = 4;

	// The CHECK_* constants are used to filter the results while loading.
	const CHECK_ACCESS = 8;
	const CHECK_IGNORE = 16;
	const CHECK_LOCKED = 32;
	const CHECK_EXPIRY = 64;
	 ...
	 */
	protected array $where = array('1=1');
	 ...
	 */
	protected array $order = array('p.id_poll DESC');
	 ...
	 */
	public function format(array $format_options = array()): array
	{
		...
		foreach ($this->choices as $i => $option)
		{
			...
			$this->formatted['choices'][$i] = array(
				'id' => 'options-' . $i,
		...
		{
			$this->formatted['buttons']['vote'] = array(
		...
		{
			$this->formatted['buttons']['results'] = array(
				'text' => 'poll_results',
		...
		{
			$this->formatted['buttons']['change_vote'] = array(
		...
		{
			$this->formatted['buttons']['lock'] = array(
		...
		{
			$this->formatted['buttons']['edit'] = array(
				'text' => 'poll_edit',
				'image' => 'poll_edit.png',
		...
		{
			$this->formatted['buttons']['remove_poll'] = array(
				'text' => 'poll_remove',
	 ...
	 */
	public static function create(array &$errors = array()): object|null
	{
		if (!self::checkCreatePermission())
			return null;
	...
	public static function vote(): void
	{
		...
		foreach ($choices as $id_choice)
		{
			...
			$inserts[] = array($poll->id, User::$me->id, $id_choice);
		}
	...
	protected function __construct(int $id = 0, int $options = 0)
	{
		...
		foreach (array(self::LOAD_BY_TOPIC, self::LOAD_BY_RECENT, self::LOAD_BY_VOTES) as $const)
	...
	protected function loadPollData(int $id, int $load_by, int &$options): void
	{
		switch ($load_by)
		{
			case self::LOAD_BY_VOTES:
				...
				$this->where = array('p.id_poll = {int:id}');
			...
			case self::LOAD_BY_TOPIC:
				...
				$this->where = array('t.id_topic = {int:id}');
				$this->params['id'] = $id;
				break;
	...
	protected function resetQueryOptions(int &$options): void
	{
		...
		foreach (array('selects', 'joins', 'where', 'order', 'params') as $var)
	...
	protected static function sanitizeInput(&$errors): void
	{
		...
		$_POST['poll_max_votes'] = min(max((int) ($_POST['poll_max_votes'] ?? 1), 1), count($_POST['options'] ?? array()));
