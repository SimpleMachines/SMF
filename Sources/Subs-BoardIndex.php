...
function getBoardIndex($board_index_options)
{
	...
	if ($board_index_options['parent_id'] != 0 && Db::$db->cte_support())
		$result_boards = Db::$db->query('', '
			WITH RECURSIVE
	...
	foreach (Db::$db->fetch_all($result_boards) as $row)
		$row_boards[$row['id_board']] = $row;

	Db::$db->free_result($result_boards);
