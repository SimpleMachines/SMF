...
class Event implements \ArrayAccess
{
	...
	const TYPE_EVENT_SIMPLE = 0;
	const TYPE_EVENT_RECURRING = 1; // Not yet implemented.
	const TYPE_BIRTHDAY = 2; // Not yet implemented.
	const TYPE_HOLIDAY = 4; // Not yet implemented.
	...
	public function fixTimezone(): void
	{
		...
		if (!isset($all_timezones[$this->timezone]))
		{
			...
			if (!$found)
			{
				$all_timezones = array($this->timezone => '[UTC' . $this->start->format($d, 'P') . '] - ' . $this->timezone) + $all_timezones;
			}
		}
	}
	...
	public function __set(string $prop, $value): void
	{
		...
		else
		{
			...
			if (in_array($prop, array('year', 'month', 'day', 'hour', 'minute', 'second')))
				$prop = 'start_' . $prop;
	 ...
	 * @return array Instances of this class for the loaded events.
	 */
	public static function loadRange(string $low_date, string $high_date, bool $use_permissions = true, array $query_customizations = array()): array
	{
		$loaded = array();
	 ...
	 */
	public static function get(string $low_date, string $high_date, bool $use_permissions = true, array $query_customizations = array())
	{
	...
	public static function create(array $eventOptions): void
	{
		...
		foreach (array('title', 'location') as $key)
	...
	protected static function queryData(array $selects, array $params = array(), array $joins = array(), array $where = array(), array $order = array(), int|string $limit = 0)
	{
		...
		while ($row = Db::$db->fetch_assoc($request))
		{
			...
			$row['member_groups'] = isset($row['member_groups']) ? explode(',', $row['member_groups']) : array();
	...
	protected static function setStartEnd(array &$eventOptions): void
	{
		...
		{
			foreach (array($eventOptions, $_POST) as &$array)
			{
				if (isset($array[$key]))
				{
		...
		foreach (array('year', 'month', 'day', 'hour', 'minute', 'second', 'date', 'time', 'datetime') as $key)
		{
		...
		foreach (array('allday', 'timezone') as $key)
		...
		$eventOptions['start'] = new Time(implode(' ', array($eventOptions['start_date'], $eventOptions['start_time'], $eventOptions['timezone'])));

		$eventOptions['end'] = new Time(implode(' ', array($eventOptions['end_date'], $eventOptions['end_time'], $eventOptions['timezone'])));
	...
	protected static function standardizeEventOptions($input): array
	{
		...
		foreach (array('start', 'end') as $var)
		{
			...
			if (empty($time_string) && isset($hour, $minute, $second))
				$time_string = implode(':', array((int) $hour, (int) $minute, (int) $second));

			if (empty($date_string) && isset($year, $month, $day))
				$date_string = implode('-', array((int) $year, (int) $month, (int) $day));
			...
			if (isset($datetime_string))
			{
				...
				if (empty($datetime_string_parsed['error_count']) && empty($datetime_string_parsed['warning_count']))
				{
					$datetime_string_parsed = array_filter(
						...
						function($key)
						{
							return in_array($key, array('year', 'month', 'day', 'hour', 'minute', 'second'));
						},
			...
			// Validate input.
			$date_is_valid = isset($month, $day, $year) && (implode('-', array($month, $day, $year)) === implode('-', array_map('intval', array($month, $day, $year)))) && checkdate($month, $day, $year);
