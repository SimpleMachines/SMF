...
class ErrorHandlerService
{
	 ...
	 */
	public array $known_error_types = [
		'general',
		'critical',
		'database',
		'cache',
	 ...
	 */
	public function handleError(int $error_level, string $error_string, string $file, int $line)
	{
		...
		$message = $this->log($error_level . ': ' . $error_string, $error_type, $file, $line);
		...
		if ($error_level % 255 == E_ERROR || $error_level % 255 == E_WARNING) {
			$this->fatal(isset(User::$me) && User::$me->allowedTo('admin_forum') ? $message : $error_string, false);
		}
	 ...
	 * @param int $line The line where the error occurred.
	 */
	public function call(int $error_level, string $error_string, string $file, int $line): void
	{
		$this->handleError($error_level, $error_string, $file, $line);
	}
	 ...
	 *
	 * Example use:
	 *  die(ErrorHandlerService::log($msg));
	 *
	 ...
	 */
	public function log(string $error_message, string|bool $error_type = 'general', string $file = '', int $line = 0, ?array $backtrace = null): string
	{
		static $last_error;
	 ...
	 *    Default: 500.
	 */
	public function fatal(string $error, string|bool $log = 'general', int $status = 500): void
	{
		...
		$this->logOnline($error);
		$this->setupFatalContext($log ? $this->log($error, $log) : $error);
	}
	 ...
	 *    Default: 'Errors'.
	 */
	public function fatalLang(string $error, string|bool $log = 'general', array $sprintf = [], int $status = 403, string $file = 'Errors'): void
	{
		...
		$this->logOnline($error, $sprintf);
		$this->setupFatalContext($error_message, $error);
	}
	 ...
	 */
	public function displayMaintenanceMessage(): void
	{
		$this->setFatalHeaders();
	 ...
	 */
	protected function logOnline(string $error, array $sprintf = []): void
	{
	 ...
	 */
	protected function setFatalHeaders(): void
	{
		if (headers_sent()) {
			return;
		}
		...
		$this->sendHttpStatus(503, 'Service Temporarily Unavailable');
	 ...
	 */
	protected function setupFatalContext(string $error_message, ?string $error_code = null): void
	{
		static $level = 0;
	 ...
	 */
	protected function logLastDatabaseError(): bool
	{
