	...
	else
	{
		...
		if (!empty($topic) && !empty($_REQUEST['quote']))
		{
			...
			$request = $smcFunc['db_query']('', '
				...
				WHERE {query_see_message} AND m.id_msg = {int:id_msg}' . (!$modSettings['postmod_active'] || allowedTo('approve_posts') ? '' : '
...
function QuoteFast()
{
	...
	$request = $smcFunc['db_query']('', '
		...
		FROM {db_prefix}messages AS m
			...
			LEFT JOIN {db_prefix}members AS mem ON (mem.id_member = m.id_member)
		WHERE {query_see_message} AND m.id_msg = {int:id_msg}' . (isset($_REQUEST['modify']) || (!empty($moderate_boards) && $moderate_boards[0] == 0) ? '' : '
