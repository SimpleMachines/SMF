...
function ShowXmlFeed()
{
	...
	foreach (array('action', 'sa', 'type', 'board', 'boards', 'c', 'u', 'limit', 'offset') as $var)
		if (isset($_GET[$var]))
			$feed_meta['self'] .= ($feed_meta['self'] === Config::$scripturl ? '?' : ';' ) . $var . '=' . $_GET[$var];
	...
	if (!empty($_GET['c']) && empty($board))
	{
		...
		if (count($_GET['c']) == 1)
		{
			...
			list ($feed_meta['title']) = Db::$db->fetch_row($request);
		...
		if ($total_cat_posts > 100 && $total_cat_posts > Config::$modSettings['totalMessages'] / 15)
			Utils::$context['optimize_msg']['lowest'] = 'm.id_msg >= ' . max(0, Config::$modSettings['maxMsgID'] - 400 - Utils::$context['xmlnews_limit'] * 5);
	}
	...
	elseif (!empty($board))
	{
		...
		if ($total_posts > 80 && $total_posts > Config::$modSettings['totalMessages'] / 10)
			Utils::$context['optimize_msg']['lowest'] = 'm.id_msg >= ' . max(0, Config::$modSettings['maxMsgID'] - 600 - Utils::$context['xmlnews_limit'] * 5);
	}
	else
	{
		$query_this_board = '{query_see_board}' . (!empty(Config::$modSettings['recycle_enable']) && Config::$modSettings['recycle_board'] > 0 ? '
			AND b.id_board != ' . Config::$modSettings['recycle_board'] : '');
		Utils::$context['optimize_msg']['lowest'] = 'm.id_msg >= ' . max(0, Config::$modSettings['maxMsgID'] - 100 - Utils::$context['xmlnews_limit'] * 5);
	}

	$feed_meta['title'] .= (!empty($feed_meta['title']) ? ' - ' : '') . Utils::$context['forum_name'];
	...
	$cachekey = array($xml_format, $_GET['action'], Utils::$context['xmlnews_limit'], $subaction);
	foreach (array('board', 'boards', 'c') as $var)
		if (isset($_GET[$var]))
			$cachekey[] = $var . '=' . implode(',', (array) $_GET[$var]);
	$cachekey = md5(Utils::jsonEncode($cachekey) . (!empty($query_this_board) ? $query_this_board : ''));
	...
	if ($xml_format == 'smf' || isset($_GET['debug']))
		header('content-type: text/xml; charset=' . (empty(Utils::$context['character_set']) ? 'UTF-8' : Utils::$context['character_set']));
	elseif ($xml_format == 'rss' || $xml_format == 'rss2')
		header('content-type: application/rss+xml; charset=' . (empty(Utils::$context['character_set']) ? 'UTF-8' : Utils::$context['character_set']));
	elseif ($xml_format == 'atom')
		header('content-type: application/atom+xml; charset=' . (empty(Utils::$context['character_set']) ? 'UTF-8' : Utils::$context['character_set']));
	elseif ($xml_format == 'rdf')
		header('content-type: ' . (BrowserDetector::isBrowser('ie') ? 'text/xml' : 'application/rdf+xml') . '; charset=' . (empty(Utils::$context['character_set']) ? 'UTF-8' : Utils::$context['character_set']));

	header('content-disposition: ' . (isset($_GET['download']) ? 'attachment' : 'inline') . '; filename="' . $filename . '.xml"');

	echo implode('', Utils::$context['feed']);
...
function fix_possible_url($val)
{
	...
	if (empty(Config::$modSettings['queryless_urls']) || (Utils::$context['server']['is_cgi'] && ini_get('cgi.fix_pathinfo') == 0 && @get_cfg_var('cgi.fix_pathinfo') == 0) || (!Utils::$context['server']['is_apache'] && !Utils::$context['server']['is_lighttpd']))
		return $val;
	...
	foreach ($data as $element)
	{
		...
		if (!empty($attrs))
		{
			foreach ($attrs as $attr_key => $attr_value)
				Utils::$context['feed']['items'] .= ' ' . $attr_key . '="' . fix_possible_url($attr_value) . '"';
		}
		...
		else
		{
			...
			// A simple string.
				...
				Utils::$context['feed']['items'] .= !empty($element['cdata']) || $forceCdata ? cdata_parse(fix_possible_url($val), $ns, $forceCdata) : fix_possible_url($val);
...
function getXmlNews($xml_format, $ascending = false)
{
	...
	while (!$done)
	{
		$optimize_msg = implode(' AND ', Utils::$context['optimize_msg']);
		$request = Db::$db->query('', '
			SELECT
		...
		// If we don't have Utils::$context['xmlnews_limit'] results, try again with an unoptimized version covering all rows.
		if ($loops < 2 && Db::$db->num_rows($request) < Utils::$context['xmlnews_limit'])
		{
			...
			Utils::$context['optimize_msg']['highest'] = 'm.id_msg <= t.id_last_msg';
			$loops++;
		}
		else
			$done = true;
...
function getXmlPosts($xml_format, $ascending = false)
{
	...
	$request = Db::$db->query('', '
		...
		array(
			'limit' => Utils::$context['xmlnews_limit'],
			'start_after' => !empty(Utils::$context['posts_start']) ? Utils::$context['posts_start'] : 0,
			'uid' => Utils::$context['xmlnews_uid'],
			'is_approved' => 1,
			'ascdesc' => !empty($ascending) ? 'ASC' : 'DESC',
		)
...
function getXmlPMs($xml_format, $ascending = false)
{
	...
	$request = Db::$db->query('', '
		...
		array(
			'limit' => Utils::$context['xmlnews_limit'],
			'start_after' => !empty(Utils::$context['personal_messages_start']) ? Utils::$context['personal_messages_start'] : 0,
			'uid' => Utils::$context['xmlnews_uid'],
			'not_deleted' => 0,
			'ascdesc' => !empty($ascending) ? 'ASC' : 'DESC',
		)
	...
	while ($row = Db::$db->fetch_assoc($request))
	{
		Utils::$context['personal_messages_start'] = $row['id_pm'];
