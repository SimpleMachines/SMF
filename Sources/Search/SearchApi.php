...
abstract class SearchApi implements SearchApiInterface
{
	 ...
	 */
	public array $blacklisted_words = array(
		'img',
		'url',
		'quote',
		'www',
	 ...
	 */
	public array $searchWords = array();
	 ...
	 */
	public array $excludedWords = array();
	 ...
	 */
	public array $excludedIndexWords = array();
	 ...
	 */
	public array $excludedSubjectWords = array();
	 ...
	 */
	public array $excludedPhrases = array();
	 ...
	 */
	public static array $default_params = array(
		'advanced' => false,
		'brd' => array(),
		'maxage' => 9999,
		'minage' => 0,
	 ...
	 * results.
	 */
	public static array $weight_factors = array(
		'frequency' => array(
	 ...
	 */
	protected array $memberlist = array();
	...
	public function compressParams(): string
	{
		...
		if (!empty($temp_params))
		{
			...
			$this->compressed_params = str_replace(array('+', '/', '='), array('-', '_', '.'), base64_encode($temp_params));
		}
		else
	...
	protected function setParams(): void
	{
		if (isset($_REQUEST['params']))
		{
			...
			{
				$this->params['brd'] = empty($this->params['brd']) ? array() : explode(',', $this->params['brd']);
			}
		}
	...
	protected function setSearchTerms(): void
	{
		...
		foreach ($orParts as $orIndex => $andParts)
		{
			...
			if ($this->supportsMethod('searchSort'))
				usort($orParts[$orIndex], array($this, 'searchSort'));
			...
			elseif ($this->params['subject_only'] && empty($this->searchWords[$orIndex]['subject_words']) && empty($this->excludedSubjectWords))
			{
				$this->errors['query_not_specific_enough'] = true;
				break;
			}
	...
	protected function setUserQuery(): void
	{
		...
		{
			$userString = strtr(Utils::htmlspecialchars($this->params['userspec'], ENT_QUOTES), array('&quot;' => '"', '%' => '\%', '_' => '\_', '*' => '%', '?' => '_'));
	...
	protected function setBoardQuery(): void
	{
		...
		{
			$this->params['brd'] = empty($_REQUEST['brd']) ? array() : $_REQUEST['brd'];
		}
		else
		{
	...
	protected function searchSubjectAndMessage()
	{
		...
		if (empty($this->params['topic']))
		{
			...
			foreach ($this->searchWords as $orIndex => $words)
			{
				...
				$ignoreRequest = Db::$db->search_query('insert_log_search_topics',
					$ignore_clause . '
				...
				if (!empty($words['indexed_words']))
				{
					...
					$search_data = array(
						...
						'params' => array(
							...
							'excluded_phrases' => !empty($this->excludedPhrases) ? $this->excludedPhrases : array(),
							'excluded_index_words' => !empty($this->excludedIndexWords) ? $this->excludedIndexWords : array(),
							'excluded_subject_words' => !empty($this->excludedSubjectWords) ? $this->excludedSubjectWords : array(),
						),
					);
		...
		if (!empty($indexedResults) || !$this->supportsMethod('indexedWordQuery', $this->getQueryParams()))
		{
			...
			$ignoreRequest = Db::$db->search_query('insert_log_search_results_no_index',
				$ignore_clause . '
				SELECT
					' . implode(',
					', $main_query['select']) . '
		...
		if ($_SESSION['search_cache']['num_results'] < Config::$modSettings['search_max_results'] && $numSubjectResults !== 0)
		{
			...
			$ignoreRequest = Db::$db->search_query('insert_log_search_results_sub_only',
				$ignore_clause . '
				SELECT
					{int:id_search},
					t.id_topic,
