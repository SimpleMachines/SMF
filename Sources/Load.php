...
function loadMemberContext($user, $display_custom_fields = false)
{
	...
	if ($context['loadMemberContext_set'] != 'minimal')
		$memberContext[$user] += array(
			...
			'avatar' => array(
				'name' => $profile['avatar'],
				'image' => $profile['avatar'] == '' ? ($profile['id_attach'] > 0 ? '<img class="avatar" src="' . (empty($profile['attachment_type']) ? $scripturl . '?action=dlattach;attach=' . $profile['id_attach'] . ';type=avatar' : $modSettings['custom_avatar_url'] . '/' . $profile['filename']) . '" alt="" />' : '') : (stristr($profile['avatar'], 'http://') || stristr($profile['avatar'], 'https://') || stristr($profile['avatar'], $modSettings['gravatar_url']) ? '<img class="avatar" src="' . (!stristr($profile['avatar'], $modSettings['gravatar_url']) ? $profile['avatar'] : str_replace('gravatar://', 'http://', $profile['avatar'])) . '"' . $avatar_width . $avatar_height . ' alt="" />' : '<img class="avatar" src="' . $modSettings['avatar_url'] . '/' . $smcFunc['htmlspecialchars']($profile['avatar']) . '" alt="" />'),
				'href' => $profile['avatar'] == '' ? ($profile['id_attach'] > 0 ? (empty($profile['attachment_type']) ? $scripturl . '?action=dlattach;attach=' . $profile['id_attach'] . ';type=avatar' : $modSettings['custom_avatar_url'] . '/' . $profile['filename']) : '') : (stristr($profile['avatar'], 'http://') || stristr($profile['avatar'], 'https://' || stristr($profile['avatar'], $modSettings['gravatar_url'])) ? (!stristr($profile['avatar'], $modSettings['gravatar_url']) ? $profile['avatar'] : str_replace('gravatar://', 'http://', $profile['avatar'])) : $modSettings['avatar_url'] . '/' . $profile['avatar']),
				'url' => $profile['avatar'] == '' ? '' : (stristr($profile['avatar'], 'http://') || stristr($profile['avatar'], 'https://') || stristr($profile['avatar'], $modSettings['gravatar_url']) ? (!stristr($profile['avatar'], $modSettings['gravatar_url']) ? $profile['avatar'] : str_replace('gravatar://', 'http://', $profile['avatar'])) : $modSettings['avatar_url'] . '/' . $profile['avatar'])
			),
