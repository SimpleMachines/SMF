...
function loadMemberContext($user, $display_custom_fields = false)
{
	...
	if ($context['loadMemberContext_set'] != 'minimal')
		$memberContext[$user] += array(
			...
			'avatar' => array(
				'name' => $profile['avatar'],
				'image' => $profile['avatar'] == '' ? ($profile['id_attach'] > 0 ? '<img class="avatar" src="' . (empty($profile['attachment_type']) ? $scripturl . '?action=dlattach;attach=' . $profile['id_attach'] . ';type=avatar' : $modSettings['custom_avatar_url'] . '/' . $profile['filename']) . '" alt="" />' : '') : (stristr($profile['avatar'], 'http://') || stristr($profile['avatar'], 'https://') || stristr($profile['avatar'], 'gravatar://www.gravatar.com/avatar/') ? '<img class="avatar" src="' . (!stristr($profile['avatar'], 'gravatar://www.gravatar.com/avatar/') ? $profile['avatar'] : str_replace('gravatar://', 'http://', $profile['avatar'])) . '" alt="" />' : '<img class="avatar" src="' . $modSettings['avatar_url'] . '/' . $smcFunc['htmlspecialchars']($profile['avatar']) . '" alt="" />'),
				'href' => $profile['avatar'] == '' ? ($profile['id_attach'] > 0 ? (empty($profile['attachment_type']) ? $scripturl . '?action=dlattach;attach=' . $profile['id_attach'] . ';type=avatar' : $modSettings['custom_avatar_url'] . '/' . $profile['filename']) : '') : (stristr($profile['avatar'], 'http://') || stristr($profile['avatar'], 'https://' || stristr($profile['avatar'], 'gravatar://www.gravatar.com/avatar/')) ? (!stristr($profile['avatar'], 'gravatar://www.gravatar.com/avatar/') ? $profile['avatar'] : str_replace('gravatar://', 'http://', $profile['avatar'])) : $modSettings['avatar_url'] . '/' . $profile['avatar']),
				'url' => $profile['avatar'] == '' ? '' : (stristr($profile['avatar'], 'http://') || stristr($profile['avatar'], 'https://') || stristr($profile['avatar'], 'gravatar://www.gravatar.com/avatar/') ? (!stristr($profile['avatar'], 'gravatar://www.gravatar.com/avatar/') ? $profile['avatar'] : str_replace('gravatar://', 'http://', $profile['avatar'])) : $modSettings['avatar_url'] . '/' . $profile['avatar'])
			),
