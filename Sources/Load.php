...
function loadUserSettings()
{
	...
	// Groups with manage_boards permission will need some special handling.
	$request = $smcFunc['db_query']('', '
		SELECT id_group
	...
	list ($context['board_manager_groups']) = $smcFunc['db_fetch_all']($request);
	$smcFunc['db_free_result']($request);
	...
	$user_info['can_manage_boards'] = !empty($user_info['is_admin']) || count(array_intersect($user_info['groups'], $context['board_manager_groups'])) > 0;
...
function loadBoard()
{
	...
	if (empty($temp))
	{
		...
		if ($smcFunc['db_num_rows']($request) > 0)
		{
			...
			$board_info['groups'] = array_unique(array_merge($row['member_groups'] == '' ? array() : explode(',', $row['member_groups']), $context['board_manager_groups']));
			$board_info['deny_groups'] = array_diff($row['deny_member_groups'] == '' ? array() : explode(',', $row['deny_member_groups']), $context['board_manager_groups']);
