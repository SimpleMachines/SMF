...
{  
	 ...
	 */
	protected static $message_limit_cache = array();
	...
	public function format(int $counter = 0, array $format_options = array()): array
	{
		...
		if ($this->member_from === User::$me->id && empty($this->received))
			$recipients['to'] = array(Lang::$txt['guest_title']);
		...
		$label_ids = array_diff(array_keys($labels), array(-1));
	...
	public static function compose(): void
	{
		...
		if (isset($_REQUEST['u']))
		{
			...
			if (isset($pm))
			{
				...
				{
					Utils::$context['recipients']['to'][$pm->member_from] = array(
						'id' => $pm->member_from,
				...
				foreach ($pm->received as $member => $received_copy)
				{
					...
					{
						Utils::$context['recipients']['to'][$received_copy->member] = array(
			...
			elseif ($_REQUEST['u'] != 'all')
			{
				foreach ($_REQUEST['u'] as $key => $member)
				{
					...
					{
						Utils::$context['recipients']['to'][User::$loaded[$member]->id] = array(
				...
				if (!empty($_REQUEST['u']))
				{
					...
					{
						Utils::$context['recipients']['to'][(int) $row['id_member']] = array(
	...
	public static function compose2(): void
	{
		...
		elseif ($is_recipient_change)
		{
			...
			self::reportErrors(array(), $namedRecipientList, $recipientList);
			return;
		}
		...
		{
			self::reportErrors($post_errors, $namesNotFound, array(
	...
	{
		$paramskey = Utils::jsonEncode(array($sort, $descending, $limit, $offset));
	 ...
	 */
	public static function reportErrors(array $error_types, array $named_recipients, array $recipient_ids = array()): void
	{
		if (!isset($_REQUEST['xml']))
		{
