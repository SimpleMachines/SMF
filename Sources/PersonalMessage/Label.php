...
class Label implements \ArrayAccess
{
	 ...
	 */
	public function __construct(int $id, string $name, array $props = array())
	{
		$this->id = $id;
		$this->name = $name;
		$this->set($props);
	...
	public static function load(): array
	{
		...
		if (User::$me->new_pm || ($labels = CacheApi::get('labelCounts:' . User::$me->id, 720)) === null)
		{
			...
			while ($row = Db::$db->fetch_assoc($result))
			{
				...
				$labels[-1]['pms'] = empty($row['pms']) ? array() : explode(',', $row['pms']);
			}
			...
			while ($row = Db::$db->fetch_assoc($result))
			{
				$labels[(int) $row['id_label']] = array(
					'id' => (int) $row['id_label'],
					'name' => $row['name'],
					'messages' => (int) $row['total'],
					'unread_messages' => (int) ($row['total'] - $row['num_read']),
					'pms' => empty($row['pms']) ? array() : array_filter(explode(',', $row['pms'])),
				);
			}
			Db::$db->free_result($result);
