...
function ViewModlog()
{
	...
	if (Utils::$context['log_type'] == 3 || $_REQUEST['action'] == 'admin')
		Utils::$context['url_start'] = '?action=admin;area=logs;sa=' . (Utils::$context['log_type'] == 3 ? 'adminlog' : 'modlog') . ';type=' . Utils::$context['log_type'];
	else
		Utils::$context['url_start'] = '?action=moderate;area=modlog;type=' . Utils::$context['log_type'];
	...
	Utils::$context['uneditable_actions'] = array('agreement_updated', 'policy_updated');

	// Handle deletion...
	if (isset($_POST['removeall']) && Utils::$context['can_delete'])
	{
		checkSession();
		...
		Db::$db->query('', '
			...
			array(
				'moderate_log' => Utils::$context['log_type'],
				'uneditable' => Utils::$context['uneditable_actions'],
			)
		);
	...
	elseif (!empty($_POST['remove']) && isset($_POST['delete']) && Utils::$context['can_delete'])
	{
		checkSession();
		validateToken('mod-ml');
	...
	Utils::$context['order'] = isset($_REQUEST['sort']) && isset($sort_types[$_REQUEST['sort']]) ? $_REQUEST['sort'] : 'time';
