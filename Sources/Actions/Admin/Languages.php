...
class Languages implements ActionInterface
{
	...
	public function download(): void
	{
		...
		if (!empty($_POST['do_install']) && !empty($_POST['copy_file'])) {
			...
			$file_status = PackageUtils::create_chmod_control($chmod_files);
			...
			elseif (!empty($install_files)) {
				PackageUtils::read_tgz_file('https://download.simplemachines.org/fetch_language.php?version=' . urlencode(SMF_VERSION) . ';fetch=' . urlencode($_GET['did']), Config::$boarddir, false, true, $install_files);
		...
		if (!isset($archive_content)) {
			$archive_content = PackageUtils::read_tgz_file('https://download.simplemachines.org/fetch_language.php?version=' . urlencode(SMF_VERSION) . ';fetch=' . urlencode($_GET['did']), null);
		}
		...
		if (!empty(Utils::$context['make_writable'])) {
			...
			$file_status = PackageUtils::create_chmod_control(Utils::$context['make_writable']);
	...
	public function editEntries()
	{
		...
		if (!empty($_POST['delete_main']) && $lang_id != 'english') {
			...
			if (!empty(Config::$modSettings['package_make_backups']) && (!isset($_SESSION['last_backup_for']) || $_SESSION['last_backup_for'] != $lang_id . '$$$')) {
				...
				$result = PackageUtils::package_create_backup('backup_lang_' . $lang_id);
