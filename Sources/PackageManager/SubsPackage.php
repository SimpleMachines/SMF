...
class SubsPackage
{
	...
	public static function parsePackageInfo(&$packageXML, $testing_only = true, $method = 'install', $previous_version = '')
	{
		...
		foreach ($actions as $action)
		{
			...
			if (in_array($actionType, array('readme', 'code', 'database', 'modification', 'redirect', 'license')))
			{
				...
				{
					$filename = self::$temp_path . '$auto_' . $temp_auto++ . (in_array($actionType, array('readme', 'redirect', 'license')) ? '.txt' : ($actionType == 'code' || $actionType == 'database' ? '.php' : '.mod'));
					self::package_put_contents($filename, $action->fetch('.'));
					$filename = strtr($filename, array(self::$temp_path => ''));
				}
				else
	...
	public static function mktree($strPath, $mode)
	{
		...
		if ($mode !== false && isset(self::$package_ftp))
			return self::$package_ftp->create_dir(strtr($strPath, array($_SESSION['pack_ftp']['root'] => '')));
		elseif ($mode === false)
		{
	...
	public static function package_chmod($filename, $perm_state = 'writable', $track_change = false)
	{
		...
		elseif (self::$package_ftp !== false && !empty($_SESSION['pack_ftp']))
		{
			$ftp_file = strtr($filename, array($_SESSION['pack_ftp']['root'] => ''));
