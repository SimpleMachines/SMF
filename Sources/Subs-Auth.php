...
{
	// Can we use SHA-3?
	$algo = in_array('sha3-512', hash_algos()) ? 'sha3-512' : 'sha512';

	// Use the salt to derive a unique key from the authentication secret.
	$secret_key = hash_pbkdf2($algo, get_auth_secret(), $salt, 4000);

	// Now use the derived key to generate an HMAC of the password.
	return hash_hmac($algo, $password, $secret_key);
}
...
if (!function_exists('hash_pbkdf2'))
{
	function hash_pbkdf2($algo, $password, $salt, $iterations, $length = 0, $raw_output = false)
	{
		...
		if (!is_numeric($iterations))
			trigger_error(__FUNCTION__ . '(): expects parameter 4 to be long, ' . gettype($iterations) . ' given', E_USER_WARNING);
		if (!is_numeric($length))
			trigger_error(__FUNCTION__ . '(): expects parameter 5 to be long, ' . gettype($length) . ' given', E_USER_WARNING);
		if ($iterations <= 0)
			trigger_error(__FUNCTION__ . '(): Iterations must be a positive integer: ' . $iterations, E_USER_WARNING);
		if ($length < 0)
