...
function build_query_board($userid)
{
	...
	else
	{
		...
		$query_part['query_see_board_join'] = 'JOIN (SELECT DISTINCT id_board FROM ' . $db_prefix . 'board_permissions_view WHERE grp IN ( '. implode(',', $groups) .') ) bpv ON (bpv.id_board = b.id_board)';
	}
