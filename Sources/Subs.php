...
function MySQLConvertOldIp($targetTable,$oldCol,$newCol)
{
	...
	$request = $smcFunc['db_query']('', 'DROP TABLE IF EXISTS {db_prefix}table ip_table');
	$smcFunc['db_free_result']($request);
	...
	$request = $smcFunc['db_query']('', 'update table {db_prefix}'.$targetTable.' a
	join {db_prefix}iptables b on a.'.$oldCol.' = b.oldip
