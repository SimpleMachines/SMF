...
$helptxt['manage_boards'] = '
	...
	On this page you can create/reorder/remove boards, and the categories above them. For example, if you had a wide-ranging site that offered information on &quot;Sports&quot; and &quot;Cars&quot; and &quot;Music&quot;, these would be the top-level Categories you would create. Under each of these categories you would likely want to create &quot;Boards&quot; for topics within each. It is a simple hierarchy, with this structure: <br>
	<ul class="normallist">
		<li>
...
$helptxt['edit_news'] = '
	<ul class="normallist">
		...
		<li>
			...
			This page allows you to send out newsletters to the members of the forum via personal message or email. First select the groups that you want to receive the newsletter, and those you don not want to receive the newsletter. If you wish, you can add additional members and email addresses that will receive the newsletter. Finally, input the message you want to send and select whether you want it to be sent to members as a personal message or as an email.
		</li>
		<li>
...
$helptxt['always_anonymize_deleted_accounts'] = 'Anonymization replaces the member’s name and email address with placeholders in post authorship data, moderation and administration logs, etc. It also replaces any mentions of the member’s name in the body text of posts.<br><br>However, anonymization will <strong>not</strong> remove any plain text occurrences of the member’s name in the body text of forum posts, nor will it remove the member’s name in the sender or recipient lines of personal messages that the member has exchanged with others.';
...
$helptxt['time_format'] = '<strong>Time Format</strong><br>
	You have the ability to adjust how the time and date look for yourself. There are a lot of little letters, but it is quite simple.
...
$helptxt['special_rrule_modifier'] = 'Use this to specify an offset from the value calculated by a special recurrence rule.<br><br>The value must be a positive or negative sign followed by an <a href="https://en.wikipedia.org/wiki/ISO_8601#Durations" class="bbc_link" target="_blank">ISO 8601 duration</a> specification. For example, "+P1D" means one day after the date calculated by the special recurrence rule, while "-P2D" means two days before.';
...
$helptxt['httponlyCookies'] = 'Cookies will not be accessible by scripting languages, such as JavaScript. This setting can help to reduce identity theft through XSS attacks. This can cause issues with third party scripts but should be on wherever possible.';
...
$helptxt['databaseSession_loose'] = 'Turning this on will decrease the bandwidth your forum uses, and make it so clicking back will not reload the page - the downside is that the (new) icons will not update, among other things. (unless you click to that page instead of going back to it).';
$helptxt['databaseSession_lifetime'] = 'This is the number of seconds for sessions to last after they have not been accessed. If a session is not accessed for too long, it is said to have &quot;timed out&quot;. Anything higher than 2400 is recommended.';
...
$helptxt['mail_type'] = 'This setting allows you to choose either PHP’s default settings, or to override those settings with SMTP. PHP does not support using authentication with SMTP (which many hosts now require), so if you want that you should select SMTP. Please note that SMTP can be slower, and some servers will not accept usernames and passwords.<br><br>You do not need to fill in the SMTP settings if this is set to PHP’s default.';
...
$helptxt['localCookies'] = 'SMF uses cookies to store login information on the client computer.
	...
	Try this setting if you are experiencing problems with users getting logged out automatically.<hr>
	Globally stored cookies are less secure when used on a shared webserver (like Tripod).<hr>
	Local cookies do not work outside the forum directory so, if your forum is stored at www.myserver.com/forum, pages like www.myserver.com/index.php cannot access the account information.<br><br>
...
$helptxt['securityDisable_credentials'] = 'When enabled, this <em>disables</em> the check made before a member adds or removes a passkey or a linked account. Those are ways into the account that outlive the session they were added from, and that a password change does not revoke, so this is not recommended.';
$helptxt['securityDisable_credentials_why'] = 'Prove who you are again. Adding a way to sign in is a lasting change to the account, so it is worth being sure it is <strong>you</strong> making it.';
...
$helptxt['quote_expand'] = 'This setting allows large quotes to be collapsed, giving the user the option to expand them to their full length. Quotes smaller than this setting will not be affected.';
...
$helptxt['db_persist'] = 'This keeps the connection active to increase performance. If you are not on a dedicated server, this may cause you problems with your host.<br>
...
$helptxt['use_ascii_slugs'] = 'Enabling this setting will cause non-ASCII characters in friendly URLs to be replaced with approximately equivalent ASCII characters. If this setting is disabled, non-ASCII letters and numbers will be left unchanged.';
...
$helptxt['countChildPosts'] = 'Checking this setting means that posts and topics in a board’s sub-board will count toward its totals on the index page.<br><br>This means that a parent board with no posts in it will not show "0" if there are any posts in its sub-boards.';
...
$helptxt['meta_keywords'] = 'These keywords are sent in the output of every page to indicate to search engines, etc., the key content of your site. They should be a comma separated list of words, and should not use HTML.<br><br>Nearly all modern search engines ignore keywords. Unless you have a specific reason to use them, you should leave this setting empty.';

$helptxt['latest_themes'] = 'This area shows a few of the latest and most popular themes from <a href="https://www.simplemachines.org/" target="_blank" rel="noopener">www.simplemachines.org</a>. It may not show up properly if your computer cannot find <a href="https://www.simplemachines.org/" target="_blank" rel="noopener">www.simplemachines.org</a>, though.';
...
$helptxt['search_why_use_index'] = 'A search index can greatly improve the performance of searches on your forum. Especially when the number of messages on a forum grows larger, searching without an index can take a long time and increase the pressure on your database. If your forum has more than 50,000 messages, you might want to consider creating a search index to assure peak performance of your forum.<br><br><ul class="bbc_list"><li>A fulltext index is a built-in index of MySQL and PostgreSQL. It’s relatively compact, but a lot of words are not indexed and it can, in some search queries, turn out to be very slow.</li><li>A parsed index is often larger than a fulltext index, but its performance is better and its results are more precise.</li></ul>';
...
$helptxt['whytwoip'] = 'Typically both IPs will be same. Sites using reverse proxies, such as those behind a CDN (Content Delivery Network), result in the CDN making the request on behalf of the user. CDNs will provide the user IP by passing it along in a untrusted header. SMF logs both of these IP addresses. You can click on either address to track that IP and ban if necessary.';
...
$helptxt['posts_and_topics'] = '
	<ul class="normallist">
		<li>
			...
			Enable the code that shows forum messages in the right layout. Also adjust which codes are allowed and which are not.
		</li><li>
...
$helptxt['alert_event_new'] = 'This will send out an alert or email as requested if there is a new calendar event added. However, if that event is posted and a topic is added, you will not get an alert for the event if you are already following that board - the alert from following the board would cover this.';
...
Anything enclosed with braces <span class="monospace">&lbrace;&rbrace;</span> will be considered as a token and will be matched against a $txt string. For example, the token <span class="monospace">&lbrace;gender&rbrace;</span> will be replaced by the value of $txt[&apos;gender&apos;]<br><br>
...
$helptxt['languages_rtl'] = 'Enabled if this language reads from right to left (e.g. Hebrew, Arabic).  Disabled if this language reads from left to right (e.g. English, Greek).';
